  ...
  render() {          
      return (
        <View style={{
            ...
            <Text style={styles2.flatListItem}>{this.props.item.name}</Text>
            <Text style={styles2.flatListItem}>{this.props.item.foodDescription}</Text>
        </View>
      );
  }
}
...
export default class PontosInfo extends Component {
  ...
  render() {
    ...
    firebase.database().ref('pontos').once('value', (data) => {
      var datinha = data.toJSON();
    })
    console.log(this.datinha);
    return (
